...
import { api, type Project, type Chat } from "@/core/api-client";
              ...
              {loading ? (
                ...
                chats.map((c) => (
                  <button
                    ...
                    className={`flex shrink-0 items-center gap-2 rounded-md px-3 py-1.5 text-left text-sm transition-colors truncate ${
                      activeChatId === c.id
                        ? "bg-primary/10 text-foreground font-medium"
                        : "text-muted-foreground hover:text-foreground hover:bg-muted"
                    }`}
